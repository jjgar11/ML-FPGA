...
Register map: hls4ml io_parallel + Vivado backend.

Preparation:
    1. Load the bitstream:  sudo ./load_overlay.sh gtsrb_gap_s_wrapper.bit ...
    2. Identify the UIO:   ls -la /sys/class/uio/  (look for the CNN IP)
...
Usage on the Ultra96:
    python3 test_scripts/gtsrb/gtsrb_axilite_infer.py --image my_sign.jpg
    python3 test_scripts/gtsrb/gtsrb_axilite_infer.py --cam 0
    python3 test_scripts/gtsrb/gtsrb_axilite_infer.py --bench     # FPGA latency

Note: the UIO_DEV and the exact offsets (REG_IN, REG_OUT, REG_CTRL)
are provided by hls4ml after synthesis — see myproject_axi.h in the HLS project.
"""
...
# ---------------------------------------------------------------------------
# AXI-Lite register map (fill in after synthesis — see myproject_axi.h)
# ap_fixed<16,6>: 6 integer bits, 10 fractional bits
# ---------------------------------------------------------------------------
UIO_DEV    = "/dev/uio0"           # update based on ls /sys/class/uio/
MAP_SIZE   = 0x100000              # 1 MB — adjust based on synthesis
REG_CTRL   = 0x00                  # ap_start on bit 0, ap_done on bit 1
REG_IN     = 0x100                 # base of the input registers (confirm)
REG_OUT    = 0x100 + N_IN * 2     # base of the 43 output values (confirm)

# ap_fixed<16,6>: fraction = 10 bits
FRAC_IN    = 10
FRAC_OUT   = 10
...
def write_inputs(mm, flat):
    """Writes 3072 ap_fixed<16,6> values as 16-bit pairs packed into 32-bit words."""
...
def fpga_predict(mm, flat):
    """Writes inputs, triggers ap_start, waits for ap_done, reads outputs."""
...
def run_image(mm, path, names):
    ...
    print(f"Image  : {path}  (FPGA latency: {lat:.1f} ms)")
...
def run_camera(mm, cam_idx, names):
    ...
    if not cap.isOpened():
        ...
        sys.exit(1)
    print(f"Camera /dev/video{cam_idx}  Ctrl+C to exit.\n")
    frame_idx = 0
    t_prev    = time.time()
    try:
        while True:
...
def run_bench(mm, n=100):
    """Measures average latency with random input."""
    flat = np.random.randn(N_IN).astype(np.float32) * 0.5
    lats = []
    for _ in range(n):
        _, _, _, lat = fpga_predict(mm, flat)
        lats.append(lat)
    print(f"FPGA latency ({n} runs): avg={np.mean(lats):.2f}ms  "
...
def main():
    ...
    print(f"Opening {args.uio}  (MAP_SIZE=0x{MAP_SIZE:X})")
    print("NOTICE: verify REG_IN/REG_OUT/REG_CTRL in myproject_axi.h before running")
