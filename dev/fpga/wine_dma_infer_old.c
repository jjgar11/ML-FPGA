/*
 * wine_dma_infer.c  —  AXI DMA inference test (Ultra96-v2, wine MLP)
 *
 * Build on Ultra96:   gcc -O2 -o wine_dma_infer wine_dma_infer.c
 * Run as root:        ./wine_dma_infer [sample_index]   (default: 0)
 *
 * Load overlay first: ./load_overlay.sh wine_axi_dma
 *
 * Prints a detailed register dump on startup so you can diagnose
 * whether AXI-Lite is alive before attempting a DMA transfer.
 */

#include <dirent.h>
#include <errno.h>
#include <fcntl.h>
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <sys/mman.h>
#include <sys/stat.h>
#include <time.h>
#include <unistd.h>

/* ---------- AXI DMA 7.1 register map ---------- */
#define MM2S_CR   0x00
#define MM2S_SR   0x04
#define MM2S_SA   0x18
#define MM2S_SA_H 0x1C
#define MM2S_LEN  0x28
#define S2MM_CR   0x30
#define S2MM_SR   0x34
#define S2MM_DA   0x48
#define S2MM_DA_H 0x4C
#define S2MM_LEN  0x58

#define CR_RS    (1u << 0)
#define CR_RESET (1u << 2)
#define SR_HALT  (1u << 0)
#define SR_IDLE  (1u << 1)
#define SR_ERR   (7u << 4)
#define SR_IOC   (1u << 12)

#define N_IN      13
#define N_OUT     3
#define IN_BYTES  (N_IN  * (int)sizeof(float))
#define OUT_BYTES (N_OUT * (int)sizeof(float))

static volatile uint32_t *dma;
#define RD(off)      (dma[(off) >> 2])
#define WR(off, v)   (dma[(off) >> 2] = (uint32_t)(v))

/* ---- wine quality test dataset (first 5 samples) ---- */
static const float SAMPLES[5][N_IN] = {
    {7.4f, 0.70f, 0.00f, 1.9f, 0.076f, 11.0f, 34.0f, 0.9978f, 3.51f, 0.56f, 9.4f, 5.0f, 0.0f},
    {7.8f, 0.88f, 0.00f, 2.6f, 0.098f, 25.0f, 67.0f, 0.9968f, 3.20f, 0.68f, 9.8f, 5.0f, 0.0f},
    {7.8f, 0.76f, 0.04f, 2.3f, 0.092f, 15.0f, 54.0f, 0.9970f, 3.26f, 0.65f, 9.8f, 5.0f, 0.0f},
    {11.2f,0.28f, 0.56f, 1.9f, 0.075f, 17.0f, 60.0f, 0.9980f, 3.16f, 0.58f, 9.8f, 6.0f, 1.0f},
    {7.4f, 0.70f, 0.00f, 1.9f, 0.076f, 11.0f, 34.0f, 0.9978f, 3.51f, 0.56f, 9.4f, 5.0f, 0.0f},
};
static const int LABELS[5] = {0, 0, 0, 1, 0};

/* ---------------------------------------------------------------------- */

static int find_uio(uint64_t target) {
    DIR *dir = opendir("/sys/class/uio");
    if (!dir) { perror("opendir /sys/class/uio"); return -1; }
    struct dirent *ent;
    while ((ent = readdir(dir))) {
        if (strncmp(ent->d_name, "uio", 3) != 0) continue;
        char path[256];
        snprintf(path, sizeof(path), "/sys/class/uio/%s/maps/map0/addr", ent->d_name);
        FILE *f = fopen(path, "r");
        if (!f) continue;
        uint64_t addr = 0;
        fscanf(f, "0x%lx", &addr);
        fclose(f);
        if (addr == target) {
            closedir(dir);
            snprintf(path, sizeof(path), "/dev/%s", ent->d_name);
            printf("[uio] DMA: %s  (base=0x%08lX)\n", path, target);
            int fd = open(path, O_RDWR | O_SYNC);
            if (fd < 0) perror("open uio");
            return fd;
        }
    }
    closedir(dir);
    fprintf(stderr, "[uio] No UIO device at 0x%08lX — load the overlay first\n", target);
    return -1;
}

static uint64_t virt_to_phys(void *v) {
    long pgsz = sysconf(_SC_PAGE_SIZE);
    uint64_t vpn = (uint64_t)(uintptr_t)v / pgsz;
    int fd = open("/proc/self/pagemap", O_RDONLY);
    if (fd < 0) { perror("pagemap"); return 0; }
    uint64_t entry = 0;
    pread(fd, &entry, 8, (off_t)(vpn * 8));
    close(fd);
    if (!(entry >> 63)) {
        fprintf(stderr, "[pagemap] page not present — mlockall may have failed\n");
        return 0;
    }
    return (entry & ((1ULL << 55) - 1)) * pgsz + ((uint64_t)(uintptr_t)v % pgsz);
}

static double mono_s(void) {
    struct timespec ts;
    clock_gettime(CLOCK_MONOTONIC, &ts);
    return ts.tv_sec + ts.tv_nsec * 1e-9;
}

static void print_sr(const char *lbl, uint32_t sr) {
    printf("  %-12s= 0x%08X  [Halt=%d Idle=%d IntErr=%d SlvErr=%d DecErr=%d IOC=%d]\n",
           lbl, sr,
           !!(sr & SR_HALT), !!(sr & SR_IDLE),
           !!(sr & (1u<<4)), !!(sr & (1u<<5)), !!(sr & (1u<<6)),
           !!(sr & SR_IOC));
}

/* Returns raw SR value (positive) or negative on error/timeout */
static int wait_ch(int off, const char *lbl, double timeout) {
    double t0 = mono_s();
    while (1) {
        uint32_t sr = RD(off);
        if (sr & SR_ERR) {
            fprintf(stderr, "[wait] %s ERROR  SR=0x%08X\n", lbl, sr);
            return -(int)sr;
        }
        if (sr & SR_IOC) return (int)sr;
        if (mono_s() - t0 > timeout) {
            fprintf(stderr, "[wait] %s TIMEOUT  SR=0x%08X\n", lbl, sr);
            return -1;
        }
    }
}

/* ---------------------------------------------------------------------- */
int main(int argc, char *argv[]) {
    int sample = 0;
    if (argc > 1) sample = atoi(argv[1]);
    if (sample < 0 || sample >= 5) { fprintf(stderr, "sample 0..4\n"); return 1; }

    /* Lock pages */
    if (mlockall(MCL_CURRENT | MCL_FUTURE) != 0)
        perror("mlockall (non-fatal)");

    /* Open DMA UIO */
    int uio_fd = find_uio(0x80000000ULL);
    if (uio_fd < 0) return 1;

    dma = mmap(NULL, 0x10000, PROT_READ | PROT_WRITE, MAP_SHARED, uio_fd, 0);
    if (dma == MAP_FAILED) { perror("mmap dma"); return 1; }

    /* ---- RAW register dump — first diagnostic ---- */
    printf("\n=== RAW registers (pre-reset) ===\n");
    uint32_t raw_mm2s_cr = RD(MM2S_CR);
    uint32_t raw_mm2s_sr = RD(MM2S_SR);
    uint32_t raw_s2mm_cr = RD(S2MM_CR);
    uint32_t raw_s2mm_sr = RD(S2MM_SR);
    printf("  MM2S_CR = 0x%08X\n", raw_mm2s_cr);
    print_sr("MM2S_SR", raw_mm2s_sr);
    printf("  S2MM_CR = 0x%08X\n", raw_s2mm_cr);
    print_sr("S2MM_SR", raw_s2mm_sr);

    if (!raw_mm2s_cr && !raw_mm2s_sr && !raw_s2mm_cr && !raw_s2mm_sr) {
        printf("\n!! TODOS LOS REGISTROS = 0  → AXI-Lite no responde\n");
        printf("   Causas tipicas:\n");
        printf("   a) ps8_0_axi_periph/M00 desconectado del DMA S_AXI_LITE\n");
        printf("   b) peripheral_aresetn=0 (dcm_locked no conectado al const_dcm_locked)\n");
        printf("   c) Overlay no cargo / bitstream incorrecto\n");
        printf("   → Correlo con fix_bd.tcl en Vivado y regenera el bitstream\n\n");
        /* Do the write-readback test anyway to confirm */
    }

    /* Write-readback test */
    printf("=== Write-readback (MM2S_CR) ===\n");
    WR(MM2S_CR, CR_RS);
    uint32_t rb = RD(MM2S_CR);
    if (rb == CR_RS)
        printf("  OK (write=0x%08X, read=0x%08X)\n", CR_RS, rb);
    else if (rb == 0)
        printf("  FALLO — AXI-Lite muerto; ver causas arriba\n");
    else
        printf("  Valor inesperado: write=0x%08X, read=0x%08X\n", CR_RS, rb);

    if (rb == 0) {
        munmap((void *)dma, 0x10000);
        close(uio_fd);
        return 2;
    }

    /* ---- Reset ---- */
    printf("\n=== Reset DMA ===\n");
    WR(MM2S_CR, CR_RESET);
    WR(S2MM_CR, CR_RESET);
    int lim = 200000;
    while ((RD(MM2S_CR) & CR_RESET) && lim--);
    lim = 200000;
    while ((RD(S2MM_CR) & CR_RESET) && lim--);
    WR(MM2S_CR, CR_RS);
    WR(S2MM_CR, CR_RS);
    print_sr("MM2S_SR", RD(MM2S_SR));
    print_sr("S2MM_SR", RD(S2MM_SR));
    if ((RD(MM2S_SR) & SR_HALT) || (RD(S2MM_SR) & SR_HALT))
        printf("  !! HALTED tras RS=1 — peripheral_aresetn probablemente 0\n");

    /* ---- Allocate DMA buffers ---- */
    uint8_t *in_buf  = mmap(NULL, 4096, PROT_READ|PROT_WRITE,
                            MAP_SHARED|MAP_ANONYMOUS|MAP_LOCKED, -1, 0);
    uint8_t *out_buf = mmap(NULL, 4096, PROT_READ|PROT_WRITE,
                            MAP_SHARED|MAP_ANONYMOUS|MAP_LOCKED, -1, 0);
    if (in_buf == MAP_FAILED || out_buf == MAP_FAILED) { perror("mmap buf"); return 1; }

    volatile uint8_t _touch;
    for (int i = 0; i < 4096; i += 64) { _touch = in_buf[i]; _touch = out_buf[i]; }
    (void)_touch;

    uint64_t in_phys  = virt_to_phys(in_buf);
    uint64_t out_phys = virt_to_phys(out_buf);
    printf("\n=== Buffers ===\n");
    printf("  in_buf  phys=0x%010lX\n", in_phys);
    printf("  out_buf phys=0x%010lX\n", out_phys);
    if (!in_phys || !out_phys) { fprintf(stderr, "virt_to_phys fallo\n"); return 1; }

    /* ---- Prepare input + sentinel ---- */
    static const float SENTINEL[N_OUT] = {1111.0f, 2222.0f, 3333.0f};
    memcpy(in_buf,  SAMPLES[sample], IN_BYTES);
    memcpy(out_buf, SENTINEL, OUT_BYTES);

    printf("\n=== Sample %d ===\n  Input: ", sample);
    const float *inp = SAMPLES[sample];
    for (int i = 0; i < N_IN; i++) printf("%.3f ", inp[i]);
    printf("\n  Expected class: %d  (0=bajo 1=medio 2=alto)\n", LABELS[sample]);

    /* ---- Clear IOC bits from previous run ---- */
    WR(MM2S_SR, SR_IOC);
    WR(S2MM_SR, SR_IOC);
    WR(MM2S_CR, CR_RS);
    WR(S2MM_CR, CR_RS);

    /* ---- Arm S2MM ---- */
    printf("\n=== Transferencia ===\n");
    WR(S2MM_DA,   (uint32_t)(out_phys & 0xFFFFFFFF));
    WR(S2MM_DA_H, (uint32_t)(out_phys >> 32));
    WR(S2MM_LEN,  OUT_BYTES);
    printf("  [1] S2MM armado (LEN=%d bytes):\n", OUT_BYTES);
    print_sr("S2MM_SR", RD(S2MM_SR));

    /* ---- Fire MM2S ---- */
    WR(MM2S_SA,   (uint32_t)(in_phys & 0xFFFFFFFF));
    WR(MM2S_SA_H, (uint32_t)(in_phys >> 32));
    WR(MM2S_LEN,  IN_BYTES);
    printf("  [2] MM2S disparado (LEN=%d bytes):\n", IN_BYTES);
    print_sr("MM2S_SR", RD(MM2S_SR));
    print_sr("S2MM_SR", RD(S2MM_SR));

    /* ---- Wait ---- */
    printf("  [3] Esperando MM2S...\n");
    int sr_mm2s = wait_ch(MM2S_SR, "MM2S", 3.0);
    printf("  MM2S final: 0x%08X  %s\n", (uint32_t)sr_mm2s,
           sr_mm2s > 0 ? "OK" : "ERROR/TIMEOUT");

    printf("  [4] Esperando S2MM...\n");
    int sr_s2mm = wait_ch(S2MM_SR, "S2MM", 3.0);
    printf("  S2MM final: 0x%08X  %s\n", (uint32_t)sr_s2mm,
           sr_s2mm > 0 ? "OK" : "ERROR/TIMEOUT");

    /* ---- Decode results ---- */
    printf("\n=== Resultados ===\n");
    float *res = (float *)out_buf;
    int beats = 0;
    for (int i = 0; i < N_OUT; i++) {
        int changed = (res[i] != SENTINEL[i]);
        printf("  out[%d] = %+9.4f  %s\n", i, res[i],
               changed ? "<- DMA escribio" : "<- SENTINEL (no escrito)");
        if (changed) beats++;
    }

    printf("\n  Beats escritos: %d/%d\n", beats, N_OUT);
    if (beats == 0 && sr_mm2s < 0)
        printf("  -> MM2S no completo: HLS IP nunca recibio datos\n"
               "     Verificar conexion DDR del MM2S en el BD\n");
    else if (beats > 0 && beats < N_OUT)
        printf("  -> TLAST bug: S2MM recibio TLAST en beat %d (esperado beat %d)\n",
               beats - 1, N_OUT - 1);
    else if (beats == N_OUT) {
        int pred = 0;
        for (int i = 1; i < N_OUT; i++)
            if (res[i] > res[pred]) pred = i;
        const char *cls[] = {"bajo", "medio", "alto"};
        printf("  Clase predicha: %d (%s)  [ground truth: %d (%s)]  %s\n",
               pred, cls[pred], LABELS[sample], cls[LABELS[sample]],
               pred == LABELS[sample] ? "CORRECTO" : "INCORRECTO");
    }

    munmap((void *)dma, 0x10000);
    munmap(in_buf,  4096);
    munmap(out_buf, 4096);
    close(uio_fd);
    return (beats > 0) ? 0 : 1;
}
